...
def img2label_paths(img_paths):
    ...
    #这里并不是在循环将标签加入list，而是复制了图片的list然后将关键字images换成labels
...
class LoadImagesAndLabels(Dataset):
    ...
    #     return self
    """
    获取单张图片，对于一个batch来说，循环调用
    """
    def __getitem__(self, index):
        index = self.indices[index]  # linear, shuffled, or image_weights
        hyp = self.hyp
        #hyp['mosaic']是采取mosaic增强的概率，也就是说首先得开启马赛克增强模式，然后随机数大于增强概率
        ...
        # Convert
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        #numpy的内存地址是不连续的，函数将一个内存不连续存储的数组转换为内存连续存储的数组，使得运行速度更快
        img = np.ascontiguousarray(img)
        #numpy To torch
