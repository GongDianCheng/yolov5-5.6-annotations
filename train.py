...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # 随机种子
    init_seeds(1 + RANK)
    # 存在子进程-分布式训练  torch_distributed_zero_first，这个函数是用来处理模型进行分布式训练时的同步问题
    #即主进程来加载数据，其他进程处于等待状态直到主进程加载完数据
    ...
    # 冻结训练的网络层
    #k是索引，v是具体的参数。
    # 如果确定freeze中的层数，有在k中的，或者说，k有在freeze中的，那么就把这层相关的v冻结掉，不再进行梯度下降和参数更新。
    ...
    # Resume 恢复训练，从ckpt里面加载数据，ckpt就是加载的模型
    ...
    # DDP mode 多加多卡，解决单机多卡主卡爆炸的问题。不过一般人用不到
    if cuda and RANK != -1:
        model = DDP(model, device_ids=[LOCAL_RANK], output_device=LOCAL_RANK)

    # Model attributes，修改模型参数
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            #前向传播时把img送到model，然后得到一个预测结果（pred），基于这个结果计算损失
            #混合精度，16位和32位
            ...
            # Optimize
            #优化的时候不是根据batch，而是根据accumalate，这个之前说过，是累计的batch，相当于放大的batch
            ...
            # Log  打印和画图
            """
            这里更新了之前初始化为zeros（shape为4）张量的mloss。mloss这个公式是计算batch平均损失，i是当前batch的索引，从0开始。
            loss items是当前batch的loss，mloss*i是之前i个batch的损失总和。最后/i+1则是计算这些当前epoch内batch的平均损失了
            """
            if RANK in (-1, 0):
...
def parse_opt(known=False):
    #argparse的用法：https://zhuanlan.zhihu.com/p/388930050
    parser = argparse.ArgumentParser()
    #weight--权重加载参数，官网提供4个预训练模型
    parser.add_argument('--weights', type=str, default=ROOT / 'yolov5s.pt', help='initial weights path')
    #cfg--网络的结构文件,比如如何定义backbone
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')
    #data--模型能识别的种类数和每种类别的名称
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='dataset.yaml path')
    #hyp--一些超参的定义，比如lr，类似C++的宏定义文件
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-low.yaml', help='hyperparameters path')
    #epochs--迭代数
    parser.add_argument('--epochs', type=int, default=300)
    #batch-size--每个用于训练的批次的图片数量
    parser.add_argument('--batch-size', type=int, default=16, help='total batch size for all GPUs, -1 for autobatch')
    #imgsz--image size，设置图像大小 640*640
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640, help='train, val image size (pixels)')
    #rect--矩形训练，yolo会把输入图像填充成正方形，而填充的部分会产生许多无意义的锚框
    #action=‘store_true’，只要运行时该变量有传参就将该变量设为True
    parser.add_argument('--rect', action='store_true', help='rectangular training')
    #resume--train中断了后继续恢复训练
    ...
    #cache--是否提前缓存到内存，以加快训练速度
    parser.add_argument('--cache', type=str, nargs='?', const='ram', help='--cache images in "ram" (default) or "disk"')
    #image-weights 使用图片采样策略
    parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training')
    #device--CPU？GPU
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    #mutl-scale 多尺度训练
    parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%')
    #single-cls single class 单一类别
    ...
    return opt
"""
当程序运行是，一般情况下，应用程序会时常通过API调用库里所预先备好的函数。
但是有些库函数却要求应用先传给它一个函数，好在合适的时候调用，以完成目标任务。这个被传入的、后又被调用的函数就称为回调函数
ref:https://blog.csdn.net/weixin_41605937/article/details/120248917
"""
def main(opt, callbacks=Callbacks()):
    ...
    if RANK in (-1, 0):
        ...
        # 缺少安装包：建议使用 pip install -i https://pypi.douban.com/simple -r requirements.txt
